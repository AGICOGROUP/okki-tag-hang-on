...
```
运行提取脚本（单次 fetch，~0.1-0.3 秒，任务级一次）→ 覆盖写本地缓存 labels-cache.json →
读缓存拆分 existing/missing → 批量应用已有标签（单次 JS 一次打多个）→
...
- **标准脚本**：`scripts/refresh_labels_cache.js`——在已登录 OKKI 页面 console 执行，单次 `GET /api/generalTagRead/list`（`credentials:'include'`，实测 115-238ms）返回全部标签名数组；脚本已固化页面结构（API 路径/响应字段/system_flag 区分系统与自定义），即插即用、无需侦察
- **任务级一次提取**：每个任务在第 1 封邮件打标签前运行脚本一次；整个任务后续所有邮件复用该次结果，绝不重复运行
- **覆盖写策略**：每次用新读取的标签列表整体替换旧缓存（含 updated_at）；新标签自然进入、已删除标签自然消失
- **缓存文件**：`labels-cache.json`（运行时生成，不入库，`.gitignore` 已排除）
...
| 2026-08-20 | 提取脚本实测：115ms 返回 18 标签（含 API 全量，覆盖写缓存） | ✅ |
| 2026-08-20 | 稳定性测试（新账户）：第 1-5 封，缓存驱动批量应用 + 新建 标签测试/新西兰/澳大利亚，零异常零重试 | ✅ 3/3×5 |
